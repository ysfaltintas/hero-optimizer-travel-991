                ...
                <CardHeader className="pb-2 p-3 sm:p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <CardTitle className="text-base sm:text-lg font-semibold text-gray-900 line-clamp-2 pr-2">
                        {hotel.name}
                      </CardTitle>
                      <div className="flex items-center mt-1 text-gray-600">
                        <MapPin size={14} className="mr-1 flex-shrink-0" />
                        <span className="text-xs sm:text-sm truncate">{hotel.location}</span>
                      </div>
                    </div>
                    <div className="flex items-center ml-2 flex-shrink-0">
                      <Star className="h-3 w-3 sm:h-4 sm:w-4 fill-yellow-400 text-yellow-400" />
                      <span className="ml-1 text-xs sm:text-sm font-medium">{hotel.rating}</span>
                    </div>
                  </div>
                </CardHeader>
                ...
                <CardContent className="pt-0 p-3 sm:p-6">
                  <div className="flex items-center justify-between">
                    <div className="text-right ml-auto">
                      <div className="text-lg sm:text-2xl font-bold text-blue-600">{hotel.price}</div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center mb-2">
                        ...
                        <span className="text-xs sm:text-sm text-gray-500">{search.date}</span>
                      </div>
                      <h3 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2 truncate">{search.destination}</h3>
                      <div className="space-y-1 text-xs sm:text-sm text-gray-600">
                        <p>{search.checkIn} - {search.checkOut}</p>
                        <p>{search.guests}</p>
                      </div>
                    </div>
                    <Button className="bg-blue-600 hover:bg-blue-700 text-white text-sm sm:text-base px-3 sm:px-4 py-2 min-h-[44px] touch-manipulation">
                      Tekrar Ara
                    </Button>
                  </div>
                </CardContent>
              ...
              <Card key={hotel.id} className="group cursor-pointer hover:shadow-lg transition-shadow duration-300 bg-white border-0">
                ...
                <CardContent className="p-4 sm:p-6">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-4 gap-3">
                    <div className="flex-1 min-w-0">
                      <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-1 line-clamp-2">{hotel.name}</h3>
                    ...
                    <div className="text-right sm:text-left">
                      <div className="text-lg sm:text-xl font-bold text-blue-600">{hotel.price}</div>
                      <div className="text-xs sm:text-sm text-gray-500">per night</div>
                    </div>
                  </div>
                  
                  <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm sm:text-base py-2.5 min-h-[44px] touch-manipulation">
                    Detayları Gör
                  </Button>
                </CardContent>
              </Card>
